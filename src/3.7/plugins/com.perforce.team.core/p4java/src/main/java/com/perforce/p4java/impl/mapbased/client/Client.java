...
public class Client extends ClientSummary implements IClient {
    ...
    @Override
    public List<IFileSpec> undoFiles(List<IFileSpec> fileSpecs, int changelistId, boolean listOnly)
            throws ConnectionException, AccessException {
        try {
            UndoFilesOptions undoFilesOptions = new UndoFilesOptions(
                    listOnly,
                    changelistId);

            return undoFiles(fileSpecs, undoFilesOptions);
    ...
    @Override
    public List<IFileSpec> undoFiles(final List<IFileSpec> fileSpecs, final UndoFilesOptions opts)
            throws P4JavaException {

        throwRequestExceptionIfPerforceServerVersionOldThanExpected(
                this.serverImpl.getServerVersion() >= 20162,
                "command requires a Perforce server version 2016.2 or later");
    ...
    public List<IFileSpec> resolveFilesAuto(List<IFileSpec> fileSpecs, ResolveFilesAutoOptions opts)
		...
		List<Map<String, Object>> resultMaps = resolveFilesAutoMap(fileSpecs, opts);
        return getIntegrationFilesFromReturn(resultMaps);
    }

	@Override
	public String resolveStreamAuto(ResolveFilesAutoOptions opts)
			throws P4JavaException {

		List<Map<String, Object>> resultMaps = resolveFilesAutoMap(null, opts);
		return parseCommandResultMapAsString(resultMaps);
	}

	private List<Map<String, Object>> resolveFilesAutoMap(List<IFileSpec> fileSpecs, ResolveFilesAutoOptions opts)
			throws P4JavaException {
		String amFlag = null;
		if ((opts == null)
				|| (!opts.isAcceptTheirs()
    ...
    @Override
    public String setStreamParentView(
            IStreamSummary.ParentView parentview, boolean sourceComments) throws P4JavaException {
        if (this.getStream() == null) {
            throwOptionsException("Missing Stream field", this);
        }
        ...
        List<Map<String, Object>> result = this.serverImpl.execMapCmdList(
                CmdSpec.STREAM,
                processParameters(null, null, opts.toArray(new String[0]), this.serverImpl),
                null);
