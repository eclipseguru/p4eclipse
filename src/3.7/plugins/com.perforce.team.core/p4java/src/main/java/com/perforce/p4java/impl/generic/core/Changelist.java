...
public class Changelist extends ChangelistSummary implements IChangelist {
	...
	protected List<IExtendedFileSpec> extendedFileSpecs = null;
	...
	public List<IExtendedFileSpec> getExtendedFiles(boolean refresh, boolean bypassServer)
		...
		// Create empty file list when currently null and bypass server
		// is true and refresh is false
		if (bypassServer && !refresh && this.extendedFileSpecs == null) {
			this.extendedFileSpecs = new ArrayList<>();
			return this.extendedFileSpecs;
		}
		...
		if (this.id == IChangelist.DEFAULT) {
			...
			this.extendedFileSpecs = defList;
		} else {
			this.extendedFileSpecs = this.serverImpl.getChangelistExtendedFiles(this.id);
		}
