	...
	public String getLoginStatus() throws P4JavaException {
		...
		if (nonNull(resultMaps) && !resultMaps.isEmpty()) {
			...
			if (isBlank(statusStr)) {
				// if message came from broker and is a nts server object the info gets returned in tagged mode
				// I think this is because the server doesn't allow us to change the tagged flag after the connection is established
