 ...
 * 
 */
public class RubyTimeLapseEditor extends NodeModelTimeLapseEditor {

    /**
     * ID
     */
    public static final String ID = "com.perforce.team.ui.ruby.RubyTimeLapseEditor"; //$NON-NLS-1$
    ...
    protected ITextAnnotateModel createTextAnnotateModel(IP4File file) {
        return new TextAnnotateModel(file, RubyNodeModel.RUBY_PREFIX);
    }
    ...
    protected NodeTickDecorator createNodeDecorator() {
        return new RubyTickDecorator(getModel());
    }
    ...
    protected IFilterNodeModel createNodeModel() {
        return new RubyNodeModel(getRevisions(), this);
    }
    ...
    protected void fillToolbar(ToolBar toolbar) {
        ...
        configureLinking(toolbar,
                Messages.RubyTimeLapseEditor_LinkWithOutlineViewSelection);
    }
    ...
    protected void updateVisibleRange() {
        if (isLinking() && !isFiltering() && this.selected != null) {
            RubyNodeModel javaModel = (RubyNodeModel) getModel();
            IModelElement element = javaModel.getRubyElement(getRevision(),
    ...
    protected void createViewer(Composite parent) {
        ...
        support.install();
        provider = new RubyFoldingProvider(viewer, support);

        RubyTextTools tools = RubyUtils.getTools();
        if (tools != null) {
            this.viewer.configure(new RubySourceViewerConfiguration(tools
    ...
    protected void dispose(IEditorInput input) {
        super.dispose(input);
        RubyUtils.getProvider().disconnect(input);
    }
    ...
    protected Range getRange(String filterKey) {
        Range range = null;
        IModelElement method = ((RubyNodeModel) getModel()).getRubyElement(
        ...
        if (method instanceof ISourceReference) {
            try {
                ...
                if (sourceRange != null) {
                    ...
                    if (content != null) {
                        // Fix for job036942, source ranges for ruby nodes
                        // contains trailing newline so remove it from the
                        // source range when found so that a newline past the
                        // "end" of the method won't be displayed
                        if (length > 2 && content.length() > 2
                                && content.endsWith("\r\n")) { //$NON-NLS-1$
                            length -= 2;
                        } else if (length > 1 && content.length() > 1
                                && (content.endsWith("\n") || content //$NON-NLS-1$
                                        .endsWith("\r"))) { //$NON-NLS-1$
                            length--;
                        }
                    }
                    range = new Range(sourceRange.getOffset(), length);
                }
            } catch (ModelException e) {
                PerforceProviderPlugin.logError(e);
            }
    ...
    protected NodeOutlinePage createOutlinePage() {
        return new RubyOutlinePage((IModelElement) getRoot(), getModel());
    }
    ...
    protected IDocument getDocument(IEditorInput input) {
        return RubyUtils.getProvider().getDocument(input);
    }
